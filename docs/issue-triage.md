...
> `.gvt-agent.json`.
>
> This is the **flat-label variant** — this repo uses GitHub's default category-label
> set (`bug`, `enhancement`, `documentation`, `duplicate`, `question`, `wontfix`) with
> **no** `type:`/`priority/`/`area:` scheme.
>
> The commands below use **GitHub Issues via the `gh` CLI**. The section headings must
> stay the same — the skill and analyst locate guidance by heading.
...
role — it is the `needsInfoLabel` in the `bugTracker` block.)
